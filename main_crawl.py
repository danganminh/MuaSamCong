...
async def get_module_1(KEY, data_base, data_id):
    driver = None
    try:
        driver = custom_driver(headless=False)
        results = module_1(driver, data_base, KEY, data_id)
        driver.quit()
...
async def get_module_1_multip_threads():
    ...
    data_id = all_df['2.1.GOI_THAU']
    threads = []
    for key in all_keys:
        if not key:
            continue
        t = threading.Thread(target=lambda: asyncio.run(get_module_1(key, data_base, data_id)))
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
...
async def get_module_2():
    ...
    try:
        ...
        for sheet in list_of_sheets:
            dict_data[sheet].to_csv(f'{sheet}.csv')
